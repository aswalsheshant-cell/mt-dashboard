...
// ---------- CHAIN MAPPING (Primary <-> Offtake name reconciliation) ----------
const CONFIRMED_MAP = [
  ['Dmart', ['D-Mart']], ['Reliance', ['Reliance Retail', 'Reliance Retail-(Azorte)']], ['Apollo', ['Apollo Healthco']],
  ['Metro Cnc', ['Metro-CNC-RRL']], ['Walmart Cnc', ['Walmart CNC']], ['Vmm', ['VMM']], ['Wh-Smith', ['WH-Smith']],
  ['Ratandeep', ['Ratnadeep']], ['Sancus(Rmt)', ['RMT-Sancus']], ['Sasta Sundar', ['Sasta Sunder']], ['Sohum', ['Sohum Shoppe']],
  ['Frankros', ['Frankross']], ['Fsn', ['Nykaa SS(fsn)']], ['Apna Mart', ['Apna Mart']], ['Arambagh', ['Arambagh']],
  ['Guardian', ['Guardian']], ['H&G', ['H&G']], ['Lulu', ['Lulu']], ['More Retail', ['More Retail']],
  ['National Mart', ['National Mart']], ['Spencer', ['Spencer']], ['Sumo Save', ['Sumo Save']], ['Trent', ['Trent']],
  ['V-Mart', ['V-Mart']], ['Vijetha', ['Vijetha']], ['Wellness Forever', ['Wellness Forever']],
];
const PROBABLE_MAP = [['BEAUTY & NUTRIE', ['B&N']]];
const UNMAPPED_PRIMARY = ['Apna Klub', 'Ascent Wellness', 'Azorte', 'Broadway', 'Deal Share', 'Eremedium',
  'G N R C Medishop Pvt Ltd', 'Grace Super MKT', 'Lifestyle', 'Lifestyle Babyshop', 'Max Hyper', 'Medanta',
  'Mother Care', 'Pothys', 'Relay', 'Sarvana', 'Shoppers Stop', "Today's Basket", 'Trilife'];

{
  const rows = [
    ['CHAIN MAPPING — Primary (DMS/billing) chain names <-> Offtake (Nielsen/sell-out panel) chain names'],
    ['Sell-Out analysis sheets use ONLY the Confirmed rows below. Probable/Unmapped chains are excluded from Sell-Out %, not guessed.'],
    [''],
    ['Offtake Chain (canonical)', 'Primary Chain Name(s)', 'Confidence', 'Notes'],
  ];
  CONFIRMED_MAP.forEach(([canon, variants]) => rows.push([canon, variants.join(' + '), 'Confirmed', '']));
  PROBABLE_MAP.forEach(([canon, variants]) => rows.push([canon, variants.join(' + '), 'Probable', 'Abbreviation match — verify with MIS before relying on it']));
  rows.push(['']);
  rows.push(['UNMAPPED — Primary-only chains (no Offtake/Nielsen counterpart found; excluded from Sell-Out analysis)']);
  UNMAPPED_PRIMARY.forEach(c => rows.push([c, '', 'Unmapped', 'No matching Offtake chain — likely outside Nielsen panel coverage']));
  rows.push(['']);
  rows.push(['ZONE MAPPING']);
  rows.push(['Primary Zone', 'Offtake Zone', 'Confidence', 'Notes']);
  ['East', 'North', 'South-1', 'South-2', 'West'].forEach(z => rows.push([z, z.toUpperCase(), 'Confirmed', 'Case-insensitive match; SUMIFS is case-insensitive']));
  rows.push(['(none)', 'PAN INDIA / FSN', 'Unmapped', 'No Primary zone equivalent — Pan-India/FSN channel excluded from Zone Sell-Out view']);
  const ws = XLSX.utils.aoa_to_sheet(rows);
  ws['!cols'] = [{ wch: 30 }, { wch: 40 }, { wch: 12 }, { wch: 60 }];
  XLSX.utils.book_append_sheet(wb, ws, 'Chain_Mapping');
}

// ---------- SELLOUT: EXECUTIVE ----------
{
  const rows = [
    ['SELL-OUT EFFICIENCY — EXECUTIVE LEVEL', '', '', ''],
    ['Sell-Out % = Offtake / Primary Billing x 100. Matched universe = 26 chains confirmed in both Primary and Offtake (see Chain_Mapping).', '', '', ''],
    [''],
    ['Metric', "Jun'26", 'Q1 FY27 (Apr-Jun26)', 'Unit'],
  ];
  const ws = XLSX.utils.aoa_to_sheet(rows);
  function primSum(monthColLetterFn, isQ1) {
    // sums Primary across ALL variant rows for ALL confirmed chains, for a given month column function
    const parts = [];
    CONFIRMED_MAP.forEach(([canon, variants]) => {
      variants.forEach(v => {
        const col = isQ1 ? 'T' : monthColLetterFn;
        parts.push(`SUMIF(${PZC}!$C:$C,"${v.replace(/"/g, '""')}",${PZC}!$${col}:$${col})`);
      });
    });
    return parts.join('+');
  }
  function offSum(monthColLetterFn, isQ1) {
    const parts = [];
    CONFIRMED_MAP.forEach(([canon]) => {
      const col = isQ1 ? 'T' : monthColLetterFn;
      parts.push(`SUMIF(${OCZ}!$B:$B,"${canon.replace(/"/g, '""')}",${OCZ}!$${col}:$${col})`);
    });
    return parts.join('+');
  }
  const junColP = pzcM('Jun-26'), junColO = oczM('Jun-26');
  set2(ws, 5, 0, 'Total Primary'); ws['B5'] = fcell(primSum(junColP, false), NUM1); ws['C5'] = fcell(primSum(null, true), NUM1); set2(ws, 5, 3, 'Rs Lacs');
  set2(ws, 6, 0, 'Total Offtake'); ws['B6'] = fcell(offSum(junColO, false), NUM1); ws['C6'] = fcell(offSum(null, true), NUM1); set2(ws, 6, 3, 'Rs Lacs');
  set2(ws, 7, 0, 'Sell-Out %'); ws['B7'] = fcell('B6/B5*100', NUM1); ws['C7'] = fcell('C6/C5*100', NUM1); set2(ws, 7, 3, '%');
  set2(ws, 8, 0, 'Billed-not-sold Gap'); ws['B8'] = fcell('B5-B6', NUM1); ws['C8'] = fcell('C5-C6', NUM1); set2(ws, 8, 3, 'Rs Lacs');
  ws['!cols'] = [{ wch: 24 }, { wch: 14 }, { wch: 18 }, { wch: 10 }];
  ws['!ref'] = 'A1:D8';
  XLSX.utils.book_append_sheet(wb, ws, 'SellOut_Executive');
}

function set2(ws, r, c, v) { ws[XLSX.utils.encode_cell({ r: r - 1, c })] = typeof v === 'string' ? { t: 's', v } : v; }

// ---------- SELLOUT: CHAIN / KAM ----------
{
  const rows = [
    ['SELL-OUT EFFICIENCY — CHAIN / KAM LEVEL (Confirmed-mapping chains only)'],
    ['Materiality filter: chains with Q1 FY27 Primary < Rs 20L are flagged "Base too small" rather than given a misleading %.'],
    [''],
    ['Offtake Chain', 'Primary Chain(s)', "Jun'26 Primary", "Jun'26 Offtake", "Jun'26 Sell-Out%", 'Q1 Primary', 'Q1 Offtake', 'Q1 Sell-Out%', 'Flag'],
  ];
  const ws = XLSX.utils.aoa_to_sheet(rows);
  let r = 5;
  CONFIRMED_MAP.forEach(([canon, variants]) => {
    const primPartsJun = variants.map(v => `SUMIF(${PZC}!$C:$C,"${v.replace(/"/g, '""')}",${PZC}!$${pzcM('Jun-26')}:$${pzcM('Jun-26')})`).join('+');
    const primPartsQ1 = variants.map(v => `SUMIF(${PZC}!$C:$C,"${v.replace(/"/g, '""')}",${PZC}!$T:$T)`).join('+');
    set2(ws, r, 0, canon);
    set2(ws, r, 1, variants.join(' + '));
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 2 })] = fcell(primPartsJun, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 3 })] = fcell(`SUMIF(${OCZ}!$B:$B,"${canon.replace(/"/g, '""')}",${OCZ}!$${oczM('Jun-26')}:$${oczM('Jun-26')})`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 4 })] = fcell(`IF(${COL(2)}${r}=0,"n/a",${COL(3)}${r}/${COL(2)}${r}*100)`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 5 })] = fcell(primPartsQ1, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 6 })] = fcell(`SUMIF(${OCZ}!$B:$B,"${canon.replace(/"/g, '""')}",${OCZ}!$T:$T)`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 7 })] = fcell(`IF(${COL(5)}${r}=0,"n/a",${COL(6)}${r}/${COL(5)}${r}*100)`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 8 })] = fcell(`IF(${COL(5)}${r}<20,"Base too small",IF(${COL(7)}${r}="n/a","n/a",IF(${COL(7)}${r}>=100,"OOS risk (Offtake>Primary)",IF(${COL(7)}${r}>=88,"Healthy","Over-stock risk"))))`, null);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 8 })].t = 's';
    delete ws[XLSX.utils.encode_cell({ r: r - 1, c: 8 })].z;
    r++;
  });
  ws['!cols'] = [{ wch: 16 }, { wch: 34 }, { wch: 13 }, { wch: 13 }, { wch: 13 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 22 }];
  ws['!ref'] = `A1:I${r - 1}`;
  XLSX.utils.book_append_sheet(wb, ws, 'SellOut_Chain_KAM');
}

// ---------- SELLOUT: ZONE ----------
{
  const rows = [
    ['SELL-OUT EFFICIENCY — ZONE LEVEL (5 common zones; Pan India/FSN excluded — no Primary zone equivalent)'],
    [''],
    ['Zone', "Jun'26 Primary", "Jun'26 Offtake", "Jun'26 Sell-Out%", 'Q1 Primary', 'Q1 Offtake', 'Q1 Sell-Out%', 'Flag'],
  ];
  const ws = XLSX.utils.aoa_to_sheet(rows);
  const ZONES = [['East', 'EAST'], ['North', 'NORTH'], ['South-1', 'SOUTH-1'], ['South-2', 'SOUTH-2'], ['West', 'WEST']];
  let r = 4;
  ZONES.forEach(([zp, zo]) => {
    set2(ws, r, 0, zp);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 1 })] = fcell(`SUMIF(${PZC}!$B:$B,"${zp}",${PZC}!$${pzcM('Jun-26')}:$${pzcM('Jun-26')})`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 2 })] = fcell(`SUMIF(${OCZ}!$C:$C,"${zo}",${OCZ}!$${oczM('Jun-26')}:$${oczM('Jun-26')})`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 3 })] = fcell(`${COL(2)}${r}/${COL(1)}${r}*100`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 4 })] = fcell(`SUMIF(${PZC}!$B:$B,"${zp}",${PZC}!$T:$T)`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 5 })] = fcell(`SUMIF(${OCZ}!$C:$C,"${zo}",${OCZ}!$T:$T)`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 6 })] = fcell(`${COL(5)}${r}/${COL(4)}${r}*100`, NUM1);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 7 })] = fcell(`IF(${COL(6)}${r}>=100,"OOS risk (Offtake>Primary)",IF(${COL(6)}${r}>=88,"Healthy","Over-stock risk"))`, null);
    ws[XLSX.utils.encode_cell({ r: r - 1, c: 7 })].t = 's';
    delete ws[XLSX.utils.encode_cell({ r: r - 1, c: 7 })].z;
    r++;
  });
  ws['!cols'] = [{ wch: 10 }, { wch: 13 }, { wch: 13 }, { wch: 13 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 22 }];
  ws['!ref'] = `A1:H${r - 1}`;
  XLSX.utils.book_append_sheet(wb, ws, 'SellOut_Zone');
}

// order: READ_ME first
wb.SheetNames = ['READ_ME', 'Executive_Synthesis', 'SellOut_Executive', 'SellOut_Chain_KAM', 'SellOut_Zone', 'Chain_Mapping', 'Corrections_Impact', 'KPI_Dashboard', 'Q1_FY27_Scorecard', 'Zone_Summary', 'Chain_Summary', 'Brand_SubCat_Summary', 'Pack_Hero', 'Primary_Summary', 'May26_External_Workings', 'Slide_Map', 'Offtake_Chain_Zone', 'Offtake_Brand_SubCat_Zone', 'Offtake_Qtr_Chain_Zone', 'Zone_Brand_Qtr', 'Primary_Zone_Chain', 'Primary_Brand_Monthly'];
wb.Workbook = { CalcPr: { fullCalcOnLoad: true } };
const out = path.join(__dirname, 'MT_Offtake_Primary_Jun26_Working_CORRECTED_V2.xlsx');
