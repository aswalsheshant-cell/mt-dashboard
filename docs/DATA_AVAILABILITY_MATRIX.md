...
|---|---|---|---|
| FY25 (Apr'24-Mar'25) | **NO_REAL_SOURCE_FOUND_IN_FULL_GIT_HISTORY (re-tested 2026-09-13)** | n/a | Verified directly against the live `dashboard/data.js`: `primary.fy_tags == ['fy26']` only, no `fy25` key. Re-tested against a full `git log --all` history search (`docs/GIT_RECOVERY_MATRIX.md`) per the standing rule to re-check before repeating an external-source-gap claim -- found only a synthetic Pareto-fallback derivation (`scripts/derive_fy25_article_primary.py`, self-documented as a proxy) that was briefly and incorrectly wired into production by a since-corrected commit, never a real SAP/ERP extract, on any of ~140 branches searched. The only FY25 series in the dashboard is Distributor Secondary (see below) -- do not read `primary.*` and conclude FY25 Primary is present. |
...
| FY25 Distributor Secondary (Apr'24-Mar'25) | AVAILABLE_AND_LOADED (national); chain+brand grain **RECOVERED_FROM_GIT_HISTORY, registered, not yet dashboard-wired** | `dashboard/data.js offtake.secondary_monthly_fy25` / `secondary_total_fy25` (national); `PowerBI/SeedData/Mapping/DistPrimary_Sheet1_FY24-25.csv` (chain+brand grain, still present in the repo, ties to the national total within 0.03% -- see `config/data_source_registry.yml fy25_secondary_chain_brand_detail` and `docs/GIT_RECOVERY_MATRIX.md`) | Rs23,332.36L across 12 months, correctly namespaced with a `secondary_` prefix inside the `offtake` block -- this is the ONLY real series covering Apr'24-Mar'25 for any of the three MT measures |
