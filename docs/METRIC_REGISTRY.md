...
| `ACCOUNT_YOY_INTELLIGENCE` | How is each chain/brand/zone growing YoY, in both value and units, and is that growth even comparable to a real prior-year base? | `qty_yoy_pct = (Qty_curr/Qty_prev - 1) x 100`; `nsv_contribution_pct = NSV_curr / SUM(NSV_curr across the dim) x 100`; `comparability` = COMPARABLE (both periods > 0) / NEW_ACCOUNT (prev = 0, curr > 0) / EXITED (curr = 0, prev > 0) / NOT_COMPARABLE (both 0) — a missing prior-year base is never read as 0% or "-100%" growth | Chain / Brand / Zone, same-period (shared-months) basis | `same_period_block()`, threaded into `scorecard_block()`'s rows | Same-period basis (`same_period_block()`), THE ONE FY RULE | Wherever a like-for-like same-period window exists | Performance & Comparison > Account & Zone Scorecard (Unit YoY / NSV Contrib% / Comparability columns; "Account growth vs contribution" bubble chart) | NEW (this pass); store-level same-store cohort tracking is a separate, unbuilt follow-on — no per-store persistent identity exists in this pipeline today |
