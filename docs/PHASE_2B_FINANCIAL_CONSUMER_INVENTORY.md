...
| F17 | P&L bridge (`buildPnl()`) hardcoded the literal `'FY26'` instead of reading `pnl_block()`'s real `fy_tag`; `D.pnl`'s defensive default shape (`{chains,totals,blended}`) didn't match the real `{by_chain,fy_tag,...}` shape | UNCONTROLLED_FINANCIAL_TRUTH (dormant) / DISPLAY_ONLY (crash risk) | **RESOLVED**, PR #211. Gate/labels now read `pl.fy_tag` dynamically; default shape corrected. |
