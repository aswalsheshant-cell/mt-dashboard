...
def brand_table(sid,x,y,w):
    ...
    data=[
     ('Dove','14.5','13.1','13.3',-120),
     ('Head & Shoulders','12.6','12.0','11.7',-89),
     ('Clinic Plus','9.7','9.6','9.7',-3),
     ("L'Oreal Paris",'8.1','9.4','9.3',119),
     ('Sunsilk','8.4','7.8','7.5',-91),
     ('Tresemme','6.1','6.6','6.8',66),
     ('Pantene','3.9','3.6','3.5',-42),
     ('Mamaearth','1.6','2.3','2.5',91),
    ]
    rowh=470000
...
shapes.append(f'<p:sp><p:nvSpPr><p:cNvPr id="{sid}" name="TitleBar"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>'
  ...
  f'<a:p>{run("NIELSEN MARKET SHARE READOUT: SHAMPOO",1900,True,WHITE)}</a:p></p:txBody></p:sp>'); sid+=1
shapes.append(textbox(sid,TL,660000,TW,300000,[run("Category-level external validation (Nielsen MS Val, Urban — Bottles)  |  standalone shampoo read, not linked to other slides",1100,False,DARK)])); sid+=1
# KPI cards
...
shapes.append(card(sid,TL,cy0,cw,ch,"Mamaearth MS Val","2.52%",
   [run("MoM ",950,False,DARK),updown('18 bps'),run("  YoY ",950,False,DARK),updown('91 bps')])); sid+=1
shapes.append(card(sid,TL+cw+gap,cy0,cw,ch,"Mamaearth Sales","₹ 6.94 Cr",
   [run("MoM ",950,False,DARK),updown('4.0%'),run("  YoY ",950,False,DARK),updown('52.2%')])); sid+=1
shapes.append(card(sid,TL+2*(cw+gap),cy0,cw,ch,"Category (Bottles)","₹ 276.0 Cr",
   [run("MoM ",950,False,DARK),updown('-3.3%'),run("  YoY ",950,False,DARK),updown('-2.8%')])); sid+=1
# table section label
ty_lbl=1800000
shapes.append(textbox(sid,TL,ty_lbl,TW,300000,[run("BRAND SHARE — NIELSEN MS VAL % (URBAN, BOTTLES)",1150,True,TEAL)])); sid+=1
# table
...
ins_runs=[run("Why it matters:  ",1100,True,TEAL),
 run("Mamaearth shampoo MS Val reached 2.52% in May’26 (",1050,False,DARK),
 run("▲ +91 bps YoY",1050,True,GREEN),
 run(", ",1050,False,DARK),run("▲ +18 bps MoM",1050,True,GREEN),
 run(") with sales of ₹6.94 Cr (",1050,False,DARK),run("▲ +52% YoY",1050,True,GREEN),
 run(") — the fastest YoY riser among scale brands. Legacy leaders ",1050,False,DARK),
 run("Dove (▼ -120 bps)",1050,True,RED),run(" and ",1050,False,DARK),
 run("Head & Shoulders (▼ -89 bps)",1050,True,RED),
 run(" are ceding share, while L’Oreal Paris (▲ +119) and Tresemme (▲ +66) also gained.",1050,False,DARK)]
action=[run("Action:  ",1100,True,TEAL),
 run("Sustain MT-chain visibility and distribution depth to convert shampoo momentum into a top-tier share position.",1050,False,DARK)]
src=[run("Source: Nielsen MS Val, Urban, Bottles — May’25 / Apr’26 / May’26.",900,False,'595959')]
...
shapes.append(f'<p:sp><p:nvSpPr><p:cNvPr id="990" name="PageNum"/><p:cNvSpPr txBox="1"/><p:nvPr/></p:nvSpPr>'
  ...
  f'{run("7 / 22",900,False,DARK)}</a:p></p:txBody></p:sp>')
...
open('w/ppt/slides/slide22.xml','w',encoding='utf-8').write(slide)
open('w/ppt/slides/_rels/slide22.xml.rels','w',encoding='utf-8').write(
...
ct=ct.replace('</Types>','<Override PartName="/ppt/slides/slide22.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slide+xml"/></Types>')
...
pr=pr.replace('</Relationships>',f'<Relationship Id="{nrid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="slides/slide22.xml"/></Relationships>')
...
pres=pres.replace('<p:sldId id="287" r:id="rId27"/>', f'<p:sldId id="{nsid}" r:id="{nrid}"/><p:sldId id="287" r:id="rId27"/>',1)
open(pp,'w',encoding='utf-8').write(pres)
print(f'Inserted Shampoo Nielsen slide as slide22.xml (rId={nrid}, sldId={nsid})')
