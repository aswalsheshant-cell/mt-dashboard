...
print(f"\nLimitations:")
print(f"  - Reliance offtake rows (no Site Code in offtake data): matched by city name only")
print(f"  - Some Maharashtra Metro-CNC entries serve both Central and West — tagged Central")
print(f"    if store name contains Vidarbha city; split/blended routes left unchanged.")
print(f"  - Pre-aggregated workbooks (.xlsx/.xlsb not in repo) require manual re-export")
print(f"    after re-running build_dashboard_data.py with the updated raw CSVs.")
