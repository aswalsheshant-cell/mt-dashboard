...
class TestBrandCounterDisclosure:
    # NOTE (root-caused 2026-09-19): the 3 tests below currently fail against
    # the checked-in dashboard/data.js, and that's expected, not a code bug.
    # load_reliance_bc_data() (build_dashboard_data.py) already sets both
    # data_complete_through and june_status -- confirmed by reading the
    # function directly. The checked-in data.js was generated by an older
    # build, before those two fields existed, so `bc` (read from that static
    # artifact) doesn't have them yet. Fixing this for real means rebuilding
    # reliance_bc from the real Reliance BC source files (gitignored, not
    # present in this environment) -- not something to fake here. Left
    # failing rather than silently skipped or weakened, so the gap stays
    # visible until a real rebuild closes it.
