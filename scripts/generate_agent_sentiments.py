...
def analyze_revenue_velocity(offtake_df: pd.DataFrame, forecast_df: pd.DataFrame | None) -> dict:
    ...
    try:
        ...
        if forecast_df is not None and len(forecast_df) > 0:
            forecast_target = forecast_df['Forecast_NSV_Lakh'].sum() if 'Forecast_NSV_Lakh' in forecast_df.columns else 0
            if forecast_target > 0:
                variance_pct = (latest_offtake / forecast_target - 1) * 100

        # Determine severity and narrative
        if variance_pct < -25 or yoy_growth < -30:
            severity = "CRITICAL"
            title = "Revenue at Critical Risk — Significantly Behind Plan"
            narrative = (
                f"Revenue stands at ₹{latest_offtake:.0f}Cr "
                f"(vs ₹{forecast_target:.0f}Cr target, {variance_pct:.1f}% variance). "
                f"YoY decline of {yoy_growth:.1f}% signals market share loss. "
                f"Immediate action required: accelerate hero SKUs, intensify trade engagement."
            )
        elif yoy_growth < -10:
            severity = "WARNING"
            title = "Revenue Deceleration — Trend Reversal Needed"
            narrative = (
                f"Revenue down {yoy_growth:.1f}% YoY to ₹{latest_offtake:.0f}Cr. "
        ...
        elif variance_pct > 15 or yoy_growth > 20:
            severity = "OPPORTUNITY"
            title = "Revenue Outperforming — Capture Upside"
            narrative = (
                f"Revenue exceeding target by {variance_pct:+.1f}% at ₹{latest_offtake:.0f}Cr. "
            ...
            severity = "ON_TRACK"
            title = "Revenue Tracking to Plan"
            narrative = (
                f"Revenue on target at ₹{latest_offtake:.0f}Cr "
                f"(vs ₹{forecast_target:.0f}Cr, {variance_pct:+.1f}% variance). "
                f"YoY growth {yoy_growth:+.1f}%, MoM acceleration {mom_growth:+.1f}%. Maintain current strategy."
            )
        ...
        return {
            ...
            "text": narrative,
            "metrics": {
                "revenue_cr": round(latest_offtake, 1),
                "mom_growth_pct": round(mom_growth, 1),
                "yoy_growth_pct": round(yoy_growth, 1),
                "forecast_variance_pct": round(variance_pct, 1),
            }
        }
    except Exception as e:
...
def analyze_pipeline_health(offtake_df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        primary = latest.get('Primary_NSV_Lakh', 0)
        offtake = latest.get('Offtake_NSV_Lakh', 0)
        conversion = latest.get('Conversion_Pct', 0)
...
def analyze_profitability(cm2_value: float, cm2_pct: float, tot_pct: float,
                          primary_nsv: float, expense_pct: float) -> dict:
    """
