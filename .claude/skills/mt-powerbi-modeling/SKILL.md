---
name: mt-powerbi-modeling
description: "Power BI modelling for the Modern Trade (MT) build kit in PowerBI/ — DAX measures, Power Query M, the star schema, the Indian-FY date table, refresh SOP, and report page layouts. Use this skill whenever the user asks for a DAX measure or formula, mentions CALCULATE / FILTER / DIVIDE / SAMEPERIODLASTYEAR / time intelligence, wants a Power Query or M transformation, asks about relationships, star schema, a new fact or dimension, RLS, a slicer or visual, the monthly refresh, a .pbix, or says 'in Power BI how do I'. Also trigger for 'the dashboard shows a wrong number in Power BI'. Do NOT use for SQL (mt-sql-analytics), pandas (mt-python-toolkit), or Excel formulas (excel-automation)."
---

# MT Power BI Modelling

Navigates and extends the existing Power BI build kit in `PowerBI/`. This is an
**extension** skill: the kit is complete and conventions are already set. Match them.
Never propose rebuilding the model.

## Boundaries — do not overlap

| Question | Skill |
|---|---|
| SQL query or warehouse layering | `mt-sql-analytics` |
| pandas / Python script | `mt-python-toolkit` |
| Excel formula or tracker | `excel-automation` |
| Numbers wrong / QC failing | `mt-error-resolution` |
| Why the business number moved | `mt-business-logic` |
| DAX, M, model, visuals, refresh | **this skill** |
...
`mt-error-resolution` — do not "fix" it by editing the measure.
...
Every Power BI answer ends with:

1. Which existing file the code goes in (or that a new numbered file is needed).
2. The DAX or M, matching the kit's formatting.
3. **Depends on:** tables, columns and relationships that must already exist.
4. **Verify:** which Data Quality card or visual confirms it, and the expected value.
