---
name: mt-python-toolkit
description: "Python and pandas recipes for Modern Trade (MT) data work — reading xlsb/xlsx/CSV, profiling and cleaning raw sales files, aggregating to a grain, YoY/MoM/contribution/rank, pivoting, merging with validation, Indian number formatting (Cr/Lac/lakh commas), and exporting to Excel or charts. Use this skill whenever the user asks for a Python script, a pandas operation, 'how do I do this in Python', wants to automate a repeated data pull, needs to clean or reshape a raw sales/offtake/primary/DMS extract, or asks about groupby, merge, pivot_table, read_excel, or dataframes. Trigger even if MT is not mentioned. Do NOT use for SQL (mt-sql-analytics), Excel formulas (excel-automation), or debugging a broken output (mt-error-resolution)."
---

# MT Python Toolkit

Copy-paste pandas recipes for Modern Trade data, written to be readable by someone who
is not a full-time programmer. Every recipe states its grain and ends with a check.

## Boundaries — do not overlap

| Question | Skill |
|---|---|
| SQL query / warehouse model | `mt-sql-analytics` |
| Excel formula or tracker | `excel-automation` |
| DAX / Power Query M | `mt-powerbi-modeling` |
| Output looks wrong, totals inflated | `mt-error-resolution` |
| Why did the number move | `mt-business-logic` |
| pandas / numpy / matplotlib code | **this skill** |
...
For any chart, load the `dataviz` skill first for colour and form choices. Mechanics:
...
Every Python answer ends with:

1. The script or snippet, with a one-line comment per non-obvious step.
2. **Grain:** what one output row represents.
3. **Checks run:** which assertions or profile results confirm the output.
4. **How to run it:** the exact command line.
