...
description: "SQL query patterns and warehouse modelling for Modern Trade (MT) analytics. Use this skill whenever the user asks for a SQL query, a JOIN / GROUP BY / window function / CTE / rank / running total / YoY / contribution query, wants to model data as staging-intermediate-mart layers, asks how to convert an Excel or Power BI calculation into SQL, needs a data-quality or reconciliation test in SQL, or says 'write me a query', 'how do I get this in SQL', 'dbt model', 'view', or 'table'. Trigger even if MT is not mentioned. Do NOT use for Excel formulas (use excel-automation) or DAX (use mt-powerbi-modeling)."
---

# MT SQL Analytics

Writes readable, grain-safe SQL for Modern Trade data and models it in layers so a
query written once can be reused. Every query this skill produces must be explainable
to a non-technical NKAM in one sentence.

## Boundaries — do not overlap

| Question | Skill to use |
|---|---|
| Excel formula, tracker, master file | `excel-automation` |
| DAX measure, Power Query M, star schema in Power BI | `mt-powerbi-modeling` |
| pandas / Python script over xlsb or CSV | `mt-python-toolkit` |
| *Why* did the number move | `mt-business-logic` |
| Output is wrong / duplicates / totals inflated | `mt-error-resolution` |
| SQL syntax, layering, tests | **this skill** |
...
## QA queries to run before publishing any result
...
-- 2. unmapped keys: must return zero rows
...
-- 3. total preservation: source total must equal mart total
select
...
-- 4. period completeness: every expected month present
select fy_tag, count(distinct fy_month) as month_count
from mart_offtake_chain_month
group by fy_tag;

-- 5. sign and range sanity
select count(*) as negative_rows from stg_offtake where offtake_value_inr < 0;
```

If any of these fail, stop and switch to `mt-error-resolution`. Do not publish.
...
Every SQL answer ends with:

1. The query, formatted to the house style.
2. **Grain:** one line stating what one output row represents.
3. **Assumptions:** table names, FY window, exclusions.
4. **QA:** which of the five QA queries to run against this result.
