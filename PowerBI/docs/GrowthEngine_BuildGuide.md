...
| Distributor Ship-To → Chain/Account validation queue (1,536 rows, sat 100% Pending, unused by any pipeline) — **RESOLVED 2026-07-14**: cross-referenced against the already-maintained, business-Confirmed `CustomerCode_Zone_State_Mapping.csv` (1,509 Confirmed rows). 580 rows matched directly (351 exact Ship-To match + 229 fuzzy), including **all 8** of the originally-flagged Low-confidence multi-chain-distributor rows. A further 937 High-confidence 1:1 direct mappings were auto-accepted per the file's own stated methodology (no Confirmed source needed for a 1:1 relationship). **19 rows genuinely have no resolution anywhere in the repo** — 2 distributor names (`CHHABRA TRADERS_Ship To`, `Sancus Networks Private Limited-RMT`) whose secondary-Cont% split across multiple chains needs real business input. | `SeedData/Mapping/ChainAccount_Mapping_Inferred.csv` (+ `_PriorityView.csv`, synced) now carry `Validation Status`/`Validated Chain`/`Validated Account`/`Validation Remarks` for all 1,517 resolved rows; the 19 unresolved are isolated in `ChainAccount_Mapping_NeedsReview.csv` |
