...
| `NPI List` | **data-derived** — `mtagent pbi derive-npi-list` generates `SeedData/Masters/NPI_List.csv` from the committed primary sell-in history (rule: first primary appearance in the latest FY = NPI; currently 32 FY27 NPIs). Editable — prune rows if the business list is narrower. Disconnected — `DAX/14_GrowthEngine_Measures.dax` §D resolves it via `TREATAS`; `compile-model` auto-includes the table + §D measures when the file exists. |
